...
# Comparison-filtered dataframe
dff = year_df.copy()
if st.session_state.countries:
    dff = dff[dff["country"].isin(st.session_state.countries)].copy()

focus_options = ["None"] + sorted(dff["country"].dropna().unique().tolist())
if st.session_state.focus_country not in focus_options:
    st.session_state.focus_country = "None"
...
with st.sidebar:
    st.selectbox(
        ...
        focus_options,
        key="focus_country",
        help="Use this instead of chart clicks in Streamlit.",
    )
...
active_country_name = None if st.session_state.focus_country == "None" else st.session_state.focus_country
active_country_code = None
if active_country_name:
    match = year_df.loc[year_df["country"] == active_country_name, "country_code"]
    if not match.empty:
        active_country_code = match.iloc[0]
...
st.markdown(
    '<div class="tip">Tip: use the Focus country control in the sidebar to highlight one country across the dashboard.</div>',
    unsafe_allow_html=True,
)
...
rank_line = (
    f"Focused country rank by total emissions: {selected_rank}"
    if selected_rank else
    "Focused country rank by total emissions: None"
)
compare_line = (
    f"Focused country is outside the top {st.session_state.top_n} emitters and has been added to the bar chart for comparison."
    if selected_outside_topn else
    f"Focused country is within the top {st.session_state.top_n} emitters."
    if selected_rank else
    "No country focused."
)
...
with st.sidebar:
    st.markdown(
        ...
        <br>Focused country: {active_country_name if active_country_name else 'None'}
        <br>{rank_line}
        <br>{compare_line}
        </div>
        """,
    ...
    st.markdown(
        ...
        Color guide: the blue scale next to the map belongs only to the choropleth and represents CO₂ per capita (t CO₂/cap/yr). In the other charts, blue shows all filtered countries, orange highlights the focused country, and sector colors identify emission sources.
        <br><br>
        Bubble size in the scatter shows total emissions (Mt CO₂/yr). The scatter excludes countries with missing GDP- or per-capita intensity values for the selected year. If a focused country is outside the top-N emitters, it is still appended to the bar chart for comparison.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
if map_df.empty:
    ...
    if focus_codes:
        focus_df = map_df[map_df["country_code"].isin(focus_codes)].copy()

        if not focus_df.empty:
            map_fig.add_trace(
                go.Choropleth(
                    locations=focus_df["country_code"],
                    z=np.ones(len(focus_df)),
                    locationmode="ISO-3",
                    ...
                    customdata=np.stack(
                        [
                            focus_df["country_code"],
                            focus_df["country"],
                            focus_df["total_mt_co2"].fillna(np.nan),
                            focus_df["co2_per_gdp_t_per_kusd"].fillna(np.nan),
                            focus_df["co2_per_capita_t"].fillna(np.nan),
                        ],
                        axis=-1,
                    ),
                    hovertemplate=(
            ...
            marker_df = focus_df[
                ["country_code", "country", "total_mt_co2", "co2_per_gdp_t_per_kusd", "co2_per_capita_t"]
            ].drop_duplicates().copy()
            marker_df = marker_df.merge(centroids, on="country_code", how="left")
            marker_df = marker_df.dropna(subset=["lat", "lon"]).copy()

            if not marker_df.empty:
                show_text = len(marker_df) <= 3
                map_fig.add_trace(
                    go.Scattergeo(
                        lon=marker_df["lon"],
                        lat=marker_df["lat"],
                        mode="markers+text" if show_text else "markers",
                        text=marker_df["country"] if show_text else None,
                        ...
                        customdata=np.stack(
                            [
                                marker_df["country_code"],
                                marker_df["country"],
                                marker_df["total_mt_co2"].fillna(np.nan),
                                marker_df["co2_per_gdp_t_per_kusd"].fillna(np.nan),
                                marker_df["co2_per_capita_t"].fillna(np.nan),
                            ],
                            axis=-1,
                        ),
                        hovertemplate=(
...
if scatter_df.empty:
    ...
    base_df = scatter_df[base_mask].copy()
    focus_df = scatter_df[scatter_df["country_code"] == active_country_code].copy() if active_country_code else pd.DataFrame()

    if not base_df.empty:
        scatter_fig.add_trace(
            go.Scatter(
                x=base_df["co2_per_gdp_t_per_kusd"],
                y=base_df["co2_per_capita_t"],
                mode="markers",
                marker=dict(
                    size=get_bubble_sizes(base_df["total_mt_co2"], min_size=8, max_size=32),
                    color=ACCENT,
                    opacity=0.55 if active_country_code else 0.75,
                    line=dict(color="white", width=0.5),
                ),
                customdata=np.stack([base_df["country_code"], base_df["country"], base_df["total_mt_co2"]], axis=-1),
                hovertemplate=(
                    "<b>%{customdata[1]}</b><br>"
    ...
    if not focus_df.empty:
        sel_x = float(focus_df["co2_per_gdp_t_per_kusd"].iloc[0])
        sel_y = float(focus_df["co2_per_capita_t"].iloc[0])
        text_pos = choose_selected_textposition(scatter_df, sel_x, sel_y)

        scatter_fig.add_trace(
            go.Scatter(
                x=focus_df["co2_per_gdp_t_per_kusd"],
                y=focus_df["co2_per_capita_t"],
                mode="markers+text",
                text=focus_df["country"],
                textposition=text_pos,
                marker=dict(
                    size=get_bubble_sizes(focus_df["total_mt_co2"], min_size=18, max_size=30),
                    color=HIGHLIGHT,
                    opacity=0.95,
                    line=dict(color="white", width=1.2),
                ),
                customdata=np.stack([focus_df["country_code"], focus_df["country"], focus_df["total_mt_co2"]], axis=-1),
                hovertemplate=(
                    "<b>%{customdata[1]}</b><br>"
                    "CO₂ per GDP: %{x:,.2f} t CO₂/kUSD/yr<br>"
                    "CO₂ per capita: %{y:,.2f} t CO₂/cap/yr<br>"
