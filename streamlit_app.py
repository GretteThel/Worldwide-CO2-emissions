...
def load_centroids(path: Path) -> pd.DataFrame:
    ...
    if not required.issubset(set(cent.columns)):
        return pd.DataFrame(columns=["country_code", "lat", "lon"])
    cent = cent[["country_code", "lat", "lon"]].copy()
...
def bar_title_text(
    ...
    if selected_outside_topn:
        return f"Top {requested_top_n} Emitters + Focus Country ({selected_year})"
...
if "focus_country_name" not in st.session_state:
    st.session_state["focus_country_name"] = "None"
...
st.markdown(
    '<div class="tip">This version uses native Streamlit Plotly rendering for reliability on Streamlit Cloud. Use the Focus country dropdown to highlight a country across the charts.</div>',
    unsafe_allow_html=True,
)
...
with sidebar_col:
    ...
    st.selectbox("Focus country (highlight)", ["None"] + COUNTRIES, key="focus_country_name")
...
focus_country_name = st.session_state["focus_country_name"]
...
if focus_country_name != "None":
    active_match = year_df.loc[year_df["country"] == focus_country_name, ["country_code", "country"]]
    if not active_match.empty:
        active_country_code = active_match.iloc[0]["country_code"]
        active_country_name = active_match.iloc[0]["country"]
...
with main_col:
    ...
    if selected_countries:
        focus_codes.update(
            year_df.loc[year_df["country"].isin(selected_countries), "country_code"]
            .dropna()
            .astype(str)
            .str.upper()
            .tolist()
    ...
    map_focus_marker_df = (
        map_focus_df[["country_code", "country"]]
        .drop_duplicates()
        .merge(centroids, on="country_code", how="left")
        .dropna(subset=["lat", "lon"])
    )

    if map_df.empty:
        map_fig = empty_figure("CO₂ per capita by country", height=500)
    else:
        map_fig = go.Figure()
        ...
        map_fig.add_trace(
            go.Choropleth(
                ...
                colorscale="Blues",
                colorbar=dict(
                    title="CO₂ per capita<br>(t CO₂/cap/yr)",
                    len=0.82,
                    y=0.5,
                    thickness=18,
                ),
                ...
                hovertemplate=(
                    "<b>%{customdata[0]}</b><br>"
                    "Total emissions: %{customdata[1]:,.1f} Mt CO₂/yr<br>"
                    "CO₂ per capita: %{customdata[3]:,.2f} t CO₂/cap/yr<br>"
                    "CO₂ per GDP: %{customdata[2]:,.2f} t CO₂/kUSD/yr"
                    "<extra></extra>"
                ),
            )
        )
        ...
        if not map_focus_marker_df.empty:
            map_fig.add_trace(
                go.Scattergeo(
                    ...
                    marker=dict(size=10, color=HIGHLIGHT, line=dict(color="white", width=1.5)),
                    hoverinfo="skip",
                    showlegend=False,
                )
            )
    ...
    if bar_df.empty:
        bar_fig = empty_figure("Top total emitters", "Mt CO₂/yr", "Country", 360)
    ...
    if scatter_df.empty:
        ...
        if not scatter_base_df.empty:
            scatter_fig.add_trace(
                go.Scatter(
                    ...
                    customdata=np.stack(
                        [scatter_base_df["country"], scatter_base_df["total_mt_co2"]],
                        axis=-1,
                    ),
                    hovertemplate=(
                        "<b>%{customdata[0]}</b><br>"
                        ...
                        "Total emissions: %{customdata[1]:,.1f} Mt CO₂/yr"
                        "<extra></extra>"
                    ),
        ...
        if not scatter_focus_df.empty:
            ...
            scatter_fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(
                        size=get_bubble_sizes(scatter_focus_df["total_mt_co2"], min_size=20, max_size=30),
                        color=HIGHLIGHT,
                        opacity=0.95,
                        line=dict(color="white", width=1.2),
                    ),
                    customdata=np.stack(
                        [scatter_focus_df["country"], scatter_focus_df["total_mt_co2"]],
                        axis=-1,
                    ),
                    hovertemplate=(
                        "<b>%{customdata[0]}</b><br>"
        ...
        scatter_fig.update_layout(
            ...
            title={
                "text": (
                    f"CO₂ intensity comparison ({selected_year})"
        ...
        scatter_fig.update_yaxes(tickformat=".2f")
    ...
    if selected_countries:
        sector_year_df = sector_year_df[sector_year_df["country"].isin(selected_countries)].copy()
    ...
    if sector_plot_df.empty or "sector" not in sector_plot_df.columns or "sector_mt_co2" not in sector_plot_df.columns:
        sector_fig = empty_figure(sector_title, "Country", "Mt CO₂/yr", 460)
    else:
        sector_order = (
    ...
    st.plotly_chart(sector_fig, use_container_width=True, config=plot_config)
