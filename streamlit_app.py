...
def click_signature(chart_name: str, point: dict) -> tuple:
    return (
        chart_name,
...
def update_focus_from_event(chart_name: str, points: list, lookups: dict):
    ...
    point = points[0]
    sig = click_signature(chart_name, point)
    ...
    st.session_state["focus_country_code"] = focus_code
    return True
...
st.markdown(
    '<div class="tip">Tip: click a country on the map, bar chart, or bubble chart to focus it across the dashboard. Use Reset filters and focus to clear the current selection.</div>',
    unsafe_allow_html=True,
)
...
with main_col:
    ...
    # marker tables for map click support
    ...
    if map_df.empty:
        ...
        # invisible centroids to make map clicks easier
